...
namespace Source.Utilities
{
    public static class Utils
    {
        ...
        /// </summary>
        public static Texture2D ToTexture2D(this RenderTexture renderTexture, int width, int height,
            UnityEngine.Experimental.Rendering.GraphicsFormat? format = null)
        {
            RenderTexture active = RenderTexture.active;
            RenderTexture.active = renderTexture;

            Texture2D tex = new Texture2D(width, height, format ?? renderTexture.graphicsFormat,
        ...
        public static RenderTexture CreateTempRenderTexture(int width, int height, Color? col = null,
            UnityEngine.Experimental.Rendering.GraphicsFormat? format = null, int antiAliasing = 1)
        {
        ...
        public static Texture2DArray CreateTextureArray(IList<Texture2D> textures, int width, int height,
            UnityEngine.Experimental.Rendering.GraphicsFormat? format = null)
        {
            Texture2DArray textureArray =
                new Texture2DArray(width, height, textures.Count, format ?? textures[0].graphicsFormat,
                    UnityEngine.Experimental.Rendering.TextureCreationFlags.None)
                {
                    filterMode = FilterMode.Bilinear,
        ...
        public static bool IsInRange(this Vector3 input, float from, float to) =>
            input.All(f => IsInRange(f, from, to));

        public static bool All(this Vector3 input, Func<float, bool> function) =>
            function(input.x) && function(input.y) && function(input.z);

        public static bool Any(this Vector3 input, Func<float, bool> function) =>
            function(input.x) || function(input.y) || function(input.z);
        ...
        public static (int x, int y, int z) PiecewiseOp(this Vector3 input, Func<float, int> function) =>
            (function(input.x), function(input.y), function(input.z));

        public static (int x, int y, int z)
            PiecewiseOp(this (int inX, int inY, int inZ) input, Func<int, int> function) =>
            (function(input.inX), function(input.inY), function(input.inZ));

        public static Vector2 PiecewiseOp(this Vector2 input, Func<float, float> function) =>
            new Vector2(function(input.x), function(input.y));

        public static Vector3 PiecewiseOp(this Vector3 input, Func<float, float> function) =>
            new Vector3(function(input.x), function(input.y), function(input.z));
        ...
        public static Vector2 Clamp(this Vector2 input, float min, float max) =>
            PiecewiseOp(input, (float f) => Mathf.Clamp(f, min, max));
        ...
        public static Vector3 CalculateAvoidanceDirection(Vector3 currentPosition, Vector3 obstaclePosition,
            Vector3 targetPosition, Vector3 up, float minimumDistance, float avoidance)
        {
            ...
            Vector3 direction;
            if (dot > 0f)
                direction = Vector3.Lerp(cross, dirToTarget,
                    Mathf.InverseLerp(minimumDistance, minimumDistance + avoidance, distanceToPlayer)).normalized;
            else
        ...
        public static Vector2 RotateAround(Vector2 point, Vector2 centre, float angle)
        {
            ...
            return centre + (new Vector2(atOrigin.x * Mathf.Cos(angle) - atOrigin.y * Mathf.Sin(angle),
                atOrigin.y * Mathf.Cos(angle) + atOrigin.x * Mathf.Sin(angle)));
        }
        ...
        public static Vector3 GetRandomDirectionInRange(Vector3 normal, float maxAngle)
        {
            ...
            Quaternion globalRot = Quaternion.FromToRotation(Vector3.up, normal) *
                                   Quaternion.Euler(xDegrees, 0f, zDegrees);
        ...
        /// Returns a string of the given array in the form [x, y, z...]
        /// </summary>
        public static string ToFormattedString<T>(this IList<T> array, int startIndex) =>
            ToFormattedString(array, startIndex, array.Count);
        ...
        public static void DrawArrow(Vector3 position, Vector3 direction, Vector3 normal, float arrowLength = 1f,
            float arrowheadScale = 0.4f)
        {
#if UNITY_EDITOR
            ...
            Handles.matrix = Matrix4x4.TRS(position + direction * arrowLength,
                Quaternion.LookRotation(direction, normal), arrowheadScale * Vector3.one);
        ...
        /// </summary>
        public static void GetBarycentricCoordinates(Vector3 p, Vector3 a, Vector3 b, Vector3 c, out float u,
        ...
        /// </summary>
        public static Vector4 BarycentricInterpolation(Vector3 p, Vector3 v1, Vector3 v2, Vector3 v3, Vector4 val1,
            Vector4 val2, Vector4 val3)
        {
        ...
        /// </summary>
        public static Color BarycentricInterpolation(Vector3 p, Vector3 v1, Vector3 v2, Vector3 v3, Color val1,
            Color val2, Color val3)
        {
            GetBarycentricCoordinates(p, v1, v2, v3, out float w1, out float w2, out float w3);
        ...
        /// </summary>
        public static Vector3 BarycentricInterpolation(Vector3 p, Vector3 v1, Vector3 v2, Vector3 v3, Vector3 val1,
            Vector3 val2, Vector3 val3)
        {
            GetBarycentricCoordinates(p, v1, v2, v3, out float w1, out float w2, out float w3);
        ...
        /// </summary>
        public static float BarycentricInterpolation(Vector3 p, Vector3 v1, Vector3 v2, Vector3 v3, float val1,
        ...
        /// </summary>
        public static float TrilinearInterpolate(Vector3 frac, float a, float b, float c, float d, float e, float f,
            float g, float h)
        {
            /*      g-------h
             *     /|      /|
        ...
        /// Interpolate values between a cube of points.
        /// </summary>
        public static Vector3 TrilinearInterpolate(Vector3 frac, Vector3 a, Vector3 b, Vector3 c, Vector3 d, Vector3 e,
            Vector3 f, Vector3 g, Vector3 h)
        {
            /*      g-------h
             *     /|      /|
             *    / |     / |
        ...
        /// </summary>
        private static Vector3 InterpolateSurfacePosition(Vector3 p, Vector3 v_0, Vector3 v_1, Vector3 v_2, Vector3 n_0,
            Vector3 n_1, Vector3 n_2)
        {
        ...
        /// </summary>
        public static bool TryFindFirstY(float from, float to, float y, Func<float, float> function, out float result,
            int iterations = 100)
        {
        ...
        public static void SortAnticlockwise(Vector3 normal, Vector3 a, Vector3 b, Vector3 c, int a_index, int b_index,
            int c_index, out Vector3 a_sorted, out Vector3 b_sorted, out Vector3 c_sorted, out int a_index_sorted,
            out int b_index_sorted, out int c_index_sorted)
        {
        ...
        public static Vector3 ProjectPointOnLineSegment(Vector3 a, Vector3 b, Vector3 point)
        {
            ...
            float pointVecMagnitudeLessThanOrEqualToDiffMagnitude =
                Mathf.Max(Mathf.Sign(diffMagnitude - pointVec.magnitude), 0f);

            return Vector3.Lerp(a, Vector3.Lerp(b, projectedPoint, pointVecMagnitudeLessThanOrEqualToDiffMagnitude),
                dotGreaterThanZero);
        }
        ...
        {
            t = Mathf.Clamp01(t);
            t = (Mathf.Sin(t * Mathf.PI * (0.2f + springiness * t * t * t)) * Mathf.Pow(1f - t, 2.2f) + t) *
                (1f + (1.2f * (1f - t)));
        ...
        /// </summary>
        public static bool LineIntersection(out Vector3 intersection, Vector3 linePoint1, Vector3 lineVec1,
        ...
        {
            bool result = LineSegmentsIntersection(new Vector2(p1.x, p1.z), new Vector2(p2.x, p2.z),
                new Vector2(p3.x, p3.z), new Vector2(p4.x, p4.z), out Vector2 flatIntersection);
        ...
        /// </summary>
        public static Vector3 ClampToLayer(this Vector3 vec, LayerMask layerMask, GameObject ignore = null,
            Vector3 dir = default)
        {
            if (dir == default)
                dir = -Vector3.up;
            ...
            int results = Physics.RaycastNonAlloc(vec - dir * CLAMP_SIZE * 0.5f, dir, m_nonAllocClampRaycastResults,
                CLAMP_SIZE, layerMask);
        ...
        /// </summary>
        public static Vector3 ClampToDefaultLayer(this Vector3 vec, GameObject ignore = null, Vector3 dir = default) =>
            ClampToLayer(vec, DefaultLayerMask, ignore, dir);
        ...
        {
            ExecuteEvents.Execute(selectable.gameObject, new PointerEventData(EventSystem.current),
        ...
        public static IEnumerable<MethodInfo> GetMethodsWithAttribute(this Type classType, Type attributeType)
        {
            return classType
                .GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic |
                            BindingFlags.DeclaredOnly).Where(methodInfo =>
